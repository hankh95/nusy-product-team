...
#### 3b. DGX Runtime Ownership & Etiquette

When working on a DGX (or any shared GPU runtime), treat long-running services (vLLM, Kanban poller, autonomous team, etc.) as **shared ship systems**:

- **Always work from a feature/exp/fix/chore branch**:
  - Branch names should follow: `feature/<card-id>-slug`, `exp/<card-id>-slug`, `fix/<card-id>-slug`, or `chore/<card-id>-slug`.
  - Never start or modify DGX services directly from `main`.
- **Require a Kanban card for runtime changes**:
  - Starting/stopping/upgrading DGX services must come from a claimed Kanban card (e.g. “DGX: start/upgrade vLLM server”).
  - Set `KANBAN_CARD_ID` in your shell when performing that work so tools can record it.
- **Use the DGX runtime lock helper for shared services**:
  - Before starting a long-running DGX service:
    - `python tools/dgx_runtime_lock.py --service vllm --action acquire`
  - When finished or on shutdown:
    - `python tools/dgx_runtime_lock.py --service vllm --action release`
  - To check who currently owns a service:
    - `python tools/dgx_runtime_lock.py --service vllm --action status`
- **Prefer wrapper scripts over raw commands**:
  - For vLLM, use `./tools/dgx_start_vllm.sh` instead of calling the deploy script directly; it will:
    - acquire a runtime lock,
    - start the standard deploy script,
    - and release the lock when the process exits.
- **Log DGX runtime changes**:
  - As part of Definition of Done for runtime-related cards:
    - Note which services were started/stopped/changed,
    - who held the lock (runtime “captain”),
    - and any issues or follow-up work discovered.
