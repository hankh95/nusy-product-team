@knowledge_capture @capability_journeyman @knowledge_scope_lake
Feature: Query note network
  ...
  Scenario: Query note network with valid inputs (happy path)
    Given the user provides valid input data for query_note_network
    When the query_note_network behavior is invoked
    Then a new Note node is queried in the knowledge graph
    And the response contains the expected output fields
    And the operation completes successfully

  Scenario: Query note network with edge case inputs (edge case)
    Given the user provides minimal or optional field data for query_note_network
    When the query_note_network behavior is invoked
  ...
  Scenario: Query note network with missing required fields (error handling)
