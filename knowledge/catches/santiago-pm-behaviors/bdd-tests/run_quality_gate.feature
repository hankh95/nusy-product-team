@quality_assurance @capability_journeyman @knowledge_scope_lake @mutates_kg
Feature: Run quality gate
  ...
  Scenario: Run quality gate with valid inputs (happy path)
    Given the user provides valid input data for run_quality_gate
    When the run_quality_gate behavior is invoked
  ...
  Scenario: Run quality gate with edge case inputs (edge case)
    Given the user provides minimal or optional field data for run_quality_gate
    When the run_quality_gate behavior is invoked
  ...
  Scenario: Run quality gate with missing required fields (error handling)
