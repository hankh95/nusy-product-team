...
Feature: Status Dashboard
  Tests for Status Dashboard behavior in santiago-pm

  @happy-path
  Scenario: Successfully status dashboard
    Given the status dashboard service is available
    Given valid input parameters are provided
    When I request to status dashboard
    Then the operation succeeds
    Then a valid response is returned
    Then the result matches expected format

  @edge-case
  Scenario: Status Dashboard with edge cases
    Given the status dashboard service is available
    Given edge case parameters are provided
    When I request to status dashboard with edge data
  ...
  @error-handling
  Scenario: Status Dashboard error handling
    Given the status dashboard service is available
    Given invalid parameters are provided
    When I request to status dashboard with invalid data
